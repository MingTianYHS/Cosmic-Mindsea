...
# `exclude:` is the only mechanism keeping a platform-specific command out of a
# platform where it cannot work. This Codex-focused distribution intentionally
# ships only the agent-skills adapter, while preserving upstream source metadata
# that still names the upstream platform set. Validate against that canonical
# vocabulary instead of treating omitted adapters as spelling errors.
validate_excludes() (
  # Subshell + its own source: adapters/lib.sh is sourced inside build_one, so
  # parse_frontmatter is not available at this scope.
  source "$REPO_ROOT/adapters/lib.sh"
  local valid; valid=" agent-skills claude-code codex-cli gemini-cli opencode hermes pi "
